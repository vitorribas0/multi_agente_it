"""Localização segura de artefatos por conversa.

Quando uma tool é executada dentro de um chat, o arquivo final fica na caixa
isolada daquela conversa. O diretório global ``exports/`` continua como
...
def artifact_dir(session: dict | None) -> Path:
    ...
    if conv_id is None:
        ...
        target = _base_dir() / "runtime" / "codex_sessions" / str(conv_id) / "artefatos"
